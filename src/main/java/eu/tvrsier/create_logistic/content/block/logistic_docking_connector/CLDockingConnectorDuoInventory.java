...
import dev.simulated_team.simulated.content.blocks.docking_connector.DockingConnectorSoloInventory;
...
public class CLDockingConnectorDuoInventory implements AbstractContainer {

    private final CLDockingConnectorSoloInventory ourInventory;
    private final CLDockingConnectorSoloInventory theirInventory;

    public CLDockingConnectorDuoInventory(
            final CLDockingConnectorSoloInventory ourInventory,
            final CLDockingConnectorSoloInventory theirInventory
    ) {
    ...
    public ItemStack insertSlot(final ItemStack stack, final int slot, final boolean simulate) {
        if (slot == 0) return this.theirInventory.insertSlot(stack, 0, simulate);
        return stack;
    }
    ...
    public ItemStack extractSlot(final int index, final int amountToExtract, final boolean simulate) {
        if (index == 1) return this.ourInventory.extractSlot(0, amountToExtract, simulate);
    ...
    @Override
    public void read(final HolderLookup.Provider provider, final CompoundTag tag) {
        throw new NotImplementedException();
    }
    ...
    public Set<ContainerSlot> getPopulatedSlots() {
        if (this.theirInventory.isEmpty()) return Set.of();
