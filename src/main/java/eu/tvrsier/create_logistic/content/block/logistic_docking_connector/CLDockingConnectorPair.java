...
public class CLDockingConnectorPair extends MagnetPair<CLDockingConnectorBlockEntity> {
    ...
    public CLDockingConnectorPair(final Level level, final BlockPos pos1, final BlockPos pos2) {
        super(level, pos1, pos2);
    }

    public static Vector3d getRelativeTipPosition(
            final CLDockingConnectorBlockEntity magnet1,
            final CLDockingConnectorBlockEntity magnet2,
            final Vector3d dist
    ) {
        final Vec3 plotPos1 = magnet1.getTipPosition();
        final Vec3 plotPos2 = magnet2.getTipPosition();

        return getRelativePosition(magnet1, magnet2, plotPos1, plotPos2, dist);
    }

    public static Vector3d getAverageTipPosition(
            final CLDockingConnectorBlockEntity magnet1,
            final CLDockingConnectorBlockEntity magnet2,
            final Vector3d dist
    ) {
        ...
        if (shell1 != null) pos1 = shell1.logicalPose().transformPosition(pos1);
        if (shell2 != null) pos2 = shell2.logicalPose().transformPosition(pos2);

        return dist.set(pos1.x + pos2.x, pos1.y + pos2.y, pos1.z + pos2.z).div(2);
    }
    ...
    private void toggleLock(
            final boolean newLock,
            final CLDockingConnectorBlockEntity dock1,
            final CLDockingConnectorBlockEntity dock2
    ) {
        final Vector3d pos = getAverageTipPosition(dock1, dock2, this.v);

        if (newLock) {
            this.level.playSound(null, pos.x, pos.y, pos.z, SimSoundEvents.DOCKING_CONNECTOR_DOCKS.event(),
                    SoundSource.BLOCKS, 1.0f, 0.9f + this.level.getRandom().nextFloat() * 0.3f);
            ...
            this.level.playSound(null, pos.x, pos.y, pos.z, SoundEvents.NETHERITE_BLOCK_FALL,
                    SoundSource.BLOCKS, 0.25f, 0.75f);
        }
    }

    public void dock(final boolean force) {
        if (!(this.level.getBlockEntity(blockPos1) instanceof final CLDockingConnectorBlockEntity dock1) ||
        !(this.level.getBlockEntity(blockPos2) instanceof final CLDockingConnectorBlockEntity dock2)) return;

        if (!dock1.isExtended() || !dock2.isExtended()) return;
        ...
        if (!force && r.lengthSquared() > minDist * minDist * 2) return;
        ...
        if (force || (t.lengthSquared() < angleTolerance * angleTolerance &&
                r.lengthSquared() < distanceTolerance * distanceTolerance)) {
            if (force) {
                locked = true;
                dock1.state = CLDockingConnectorBlockEntity.DockingConnectorState.LOCKING;
                dock2.state = CLDockingConnectorBlockEntity.DockingConnectorState.LOCKING;
            } else {
                final boolean previousLockingState = dock1.state ==
                        CLDockingConnectorBlockEntity.DockingConnectorState.LOCKED &&
                        dock2.state == CLDockingConnectorBlockEntity.DockingConnectorState.LOCKED;
                if (previousLockingState != locked) this.toggleLock(locked, dock1, dock2);
            }

            if (!this.level.isClientSide) {
                final ServerSubLevel first = (ServerSubLevel) Sable.HELPER.getContaining(dock1);
                final ServerSubLevel second = (ServerSubLevel) Sable.HELPER.getContaining(dock2);

                if (first != second) {
                    final boolean f;
                    if (first != null ^ second != null) f = first != null;
                    else f = true;
                    ...
                    final Quaterniond orientation = this.target.mul(blockOrientation1, new Quaterniond()).
                            mul(blockOrientation2.conjugate());

                    this.orientation1.set(
                            first == null ? this.orientation1.identity() : first.logicalPose().orientation());
                    this.orientation2.set(
                            second == null ? this.orientation2.identity() : second.logicalPose().orientation());
                    this.orientation2.transformInverse(r);
                    this.orientation2.premul(this.orientation1.conjugate());

                    dock1.setDock(dock2, locked, f ? orientation : null, r, this.orientation2);
                    dock2.setDock(dock1, locked, f ? null : orientation.conjugate(), r.negate(),
                            this.orientation2.conjugate());
                    return;
                }
            ...
            if (dock1.state == CLDockingConnectorBlockEntity.DockingConnectorState.LOCKING &&
                dock2.state == CLDockingConnectorBlockEntity.DockingConnectorState.LOCKING) unDock();
        }
    }

    public void unDock() {
        final BlockEntity be1 = this.level.getBlockEntity(blockPos1);
        final BlockEntity be2 = this.level.getBlockEntity(blockPos2);

        if (be1 instanceof final CLDockingConnectorBlockEntity dock1 &&
                be2 instanceof final CLDockingConnectorBlockEntity dock2) {
            dock1.unDock();
            dock2.unDock();
    ...
    @Override
    protected double forceDistanceScale(double distance,
            final CLDockingConnectorBlockEntity magnet1,
            final CLDockingConnectorBlockEntity magnet2
    ) {
    ...
    @Override
    protected double torqueDistanceScale(double distance,
            final CLDockingConnectorBlockEntity magnet1,
            final CLDockingConnectorBlockEntity magnet2
    ) {
        final double scale = this.preventPairFightingScale(distance, magnet1, magnet2);
    ...
    protected double preventPairFightingScale(
